...
class TransportsWater : Model, Transport {
    ...
    override val notes: ObservableList<Note> by lazy {
        ...
        params["itemType"] = "Rules::Transports::Water"
        params["itemId"] = id
    ...
    fun getUnpowered(): String = if (unpowered) FX.messages["yes"] else FX.messages["no"]

    override fun addToCharacter(character: Character) {
        CharactersTransportsWater(character.id, id, count).create()
    }
    ...
    override fun removeToCharacter(character: Character) {
        ...
        CharactersTransportsWater().find_by(params).destroy()
    }
}
